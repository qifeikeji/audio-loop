...
### 常见问题：插入普通 AUX 线，"3.5mm 音频输入"卡片没反应

这是**硬件电气特性**导致的，不是软件 bug：

- 普通 "AUX 音频线"（比如车载 AUX 线）是 **3 极 (TRS)**：左声道 / 右声道 / 地。
- 手机耳机孔是 **4 极 (TRRS)**：左声道 / 右声道 / 地 / 麦克风。
- 把 3 极插头插进 4 极插孔时，麦克风触点通常会被插头的金属套短路或悬空，手机的麦克风检测电路检测不到"有效麦克风阻抗"，于是系统直接判定为"纯耳机输出（无麦克风）"，根本不会在输入设备列表里出现——所以 App 检测不到，属于正常现象。

**排查方法**：App 首页有一个"查看设备详情"按钮，点开会弹出系统当前识别到的所有音频输入/输出设备列表。插入 AUX 线后点一下，如果输出列表里出现了"有线耳机(无麦克风)"，就印证了上面的原因。

**解决方案（任选其一）**：

1. **换用带麦克风识别的四段音频线**：某些"耳机转接线 + 麦克风"套装、直播声卡自带的四段线可以让手机识别为"有线耳机(带麦克风)"。也可以自制一根在麦克风脚和地之间焊接约 2.2kΩ 电阻的转接头（俗称"麦克风假负载"），让手机误以为接了带麦的耳机，从而开放输入通道。
2. **改用 USB 声卡（推荐，最稳定）**：买一个几十元的 USB Type-C 外置声卡（自带 3.5mm Line-in/麦克风口），通过手机的 USB-C 口连接。这类设备会被识别为标准 USB 音频设备，兼容性远好于耳机孔的麦克风识别方案。本 App 已经把 `USB 设备` / `USB 耳机` / `模拟 Line-in` 也纳入了"输入检测"范围，换成 USB 声卡后无需改代码即可直接使用。
3. 如果暂时没有合适的线材，可以先用"查看设备详情"确认现象，再决定采购哪种线/设备。
